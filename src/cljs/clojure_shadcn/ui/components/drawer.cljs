...
(defn drawer-trigger
  [raw-props & children]
  (into
   [:> drawer-trigger-primitive (assoc (normalize-props raw-props) :data-slot "drawer-trigger")]
   children))

(defn drawer-close
  [raw-props & children]
  (into [:> drawer-close-primitive (assoc (normalize-props raw-props) :data-slot "drawer-close")]
        children))
