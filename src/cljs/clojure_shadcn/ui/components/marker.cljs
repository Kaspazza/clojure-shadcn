...
(defn marker
  [{:keys [variant as-child? class]
    :or {variant :default}
    :as props}
   &
   children]
  (let [p (-> props
              (assoc :data-slot "marker" :data-variant (name variant))
...
(defn marker-icon
  ...
  (into [:span
         (-> props
             (assoc :data-slot "marker-icon"
                    :aria-hidden true
                    :class (merge-classes "size-4 shrink-0 [&_svg:not([class*='size-'])]:size-4"
                                          class))
             (dissoc :class-name))]
        children))
