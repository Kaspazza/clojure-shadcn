...
(defn- cljs-data [value] (when-not (nil? value) (js->clj value :keywordize-keys true)))
    ...
    (into [:>
           ...
           (-> props
               (assoc
                ...
                :onLayoutChanged
                (when callback (fn [layout meta] (callback (cljs-data layout) (cljs-data meta))))
                :className (merge-classes "flex h-full w-full aria-[orientation=vertical]:flex-col"
                                          class))
