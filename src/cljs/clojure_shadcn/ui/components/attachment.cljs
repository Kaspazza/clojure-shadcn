...
(defn attachment
  ...
  (node
   ...
   (-> props
       (assoc :data-state (name state) :data-size (name size) :data-orientation (name orientation))
       (dissoc :state :size :orientation))
   children))
...
(defn attachment-action
  ...
  (apply button/button
         (-> props
             (assoc :data-slot "attachment-action" :variant variant :size size :class class)
             (dissoc :class-name))
         children))

(defn attachment-trigger
  [{:keys [as-child? type class]
    :as props}
   &
   children]
