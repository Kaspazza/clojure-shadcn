...
(defn h3
  [props & children]
  (text-element :h3 "scroll-m-20 text-2xl font-semibold tracking-tight" props children))

(defn h4
  [props & children]
  (text-element :h4 "scroll-m-20 text-xl font-semibold tracking-tight" props children))

(defn p [props & children] (text-element :p "leading-7 [&:not(:first-child)]:mt-6" props children))

(defn blockquote
  [props & children]
  (text-element :blockquote "mt-6 border-l-2 pl-6 italic" props children))
...
(defn small
  [props & children]
  (text-element :small "text-sm leading-none font-medium" props children))

(defn muted [props & children] (text-element :p "text-muted-foreground text-sm" props children))

(defn table
  [props & children]
  [:div {:data-slot "typography-table"
         :class "my-6 w-full overflow-y-auto"}
