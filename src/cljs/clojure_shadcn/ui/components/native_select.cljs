...
(defn- option-element
  [tag slot raw-props children]
  (let [{:keys [class]
         :as props}
        (normalize-props raw-props)]
    (into [tag
           (-> props
               (assoc :data-slot slot :class (merge-classes "bg-[Canvas] text-[CanvasText]" class))
               (dissoc :class-name))]
          children)))

(defn native-select-option
  [props & children]
  (option-element :option "native-select-option" props children))

(defn native-select-optgroup
  [props & children]
  (option-element :optgroup "native-select-optgroup" props children))
