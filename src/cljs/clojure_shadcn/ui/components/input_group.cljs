...
(defn input-group-addon
  [{:keys [class align on-click]
    :or {align :inline-start}
    :as props}
   &
   children]
  (let
    ...
    (into [button/button
           (-> props
               (assoc :type (or (:type props) "button")
                      :size :xs
                      :variant (or (:variant props) :ghost)
                      :data-size (name size)
                      :class (merge-classes "flex items-center gap-1 text-sm shadow-none"
                                            size-class
                                            class))
               (dissoc :class-name))]
          children)))
