...
(defstory ApiReference []
  (r/as-element (helpers/wrap-component
    [:div {:class "space-y-4 p-6 max-w-4xl"}
     [helpers/api-component-card {:component-name "native-select" :description "Native HTML select wrapped for styling with an aria-hidden chevron. Browser semantics, keyboard interaction, form submission, and option constraints remain native." :props [{:name ":size" :type ":default | :sm" :default ":default" :description "Controls select height and padding; consumed as a styling data attribute."} {:name ":class" :type "string" :default nil :description "Classes merged onto the select, not its wrapper."} {:name "select props" :type "map entries" :default nil :description "Normalized and forwarded to select, including :name, :value, :default-value, :on-change, :disabled, :required, and ARIA props."}]}]
     [helpers/api-component-card {:component-name "native-select-option / native-select-optgroup" :description "Styled native option and optgroup elements. Nest both beneath native-select; optgroup should contain options and receive a :label." :props [{:name ":class / native props" :type "string / map entries" :default nil :description "Classes are merged and remaining normalized props forwarded to option or optgroup."}]}]])))
