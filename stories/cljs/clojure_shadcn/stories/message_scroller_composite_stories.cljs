...
(def ^:private queued-messages
  [{:id 1
   ...
   {:id 2
    :role :assistant
    :text "That's the classic streaming scroll problem. Message Scroller follows new content only while the reader is already at the bottom. The moment they scroll up, auto-scroll backs off and preserves their position."}
   {:id 3
    :role :user
   ...
   {:id 4
    :role :assistant
    :text "Message Scroller Item fixes that with turn anchoring. Mark the user turn as the scroll anchor and the reply starts in view without blindly snapping the entire document to the bottom."}
   {:id 5
    :role :user
    :text "And if they've scrolled up to re-read an older answer? I don't want to yank them back down."}
   {:id 6
    :role :assistant
    :text "You won't. When unseen content arrives, Message Scroller Button appears at the bottom. One click returns to the newest message and re-engages auto-scroll — quiet when you're caught up, helpful when you're not."}
   {:id 7
    :role :user
    :text "Last one — does this work with assistive tech?"}
   {:id 8
    :role :assistant
    :text "Message Scroller Content is exposed as a live log so screen readers can announce additions. The scroll control is a real button and disappears from the tab order when it isn't needed."}])
         ...
         [:div {:class "relative flex flex-col gap-4"}
          [card/card {:class "mx-auto h-[35rem] w-full max-w-sm gap-0 py-0"}
           [card/card-header {:class "gap-1 border-b py-5"}
            [card/card-title {} "New Chat"]
            [card/card-description {} "How can I help you today?"]
            [card/card-action {}
             [tooltip/tooltip
              ...
              [empty/empty {:class "h-full border-0"}
               [empty/empty-header {}
                [empty/empty-media {:variant :icon}
                 [:> MessageCircleDashedIcon]]
                [empty/empty-title {} "Morning, shadcn!"]
           ...
           [card/card-footer {:class "flex-col gap-2 py-5"}
            [:form {:class "w-full" :on-submit send-next!}
             [input-group/input-group {:class "h-auto flex-col items-stretch"}
              ...
              [input-group/input-group-addon {:align :block-end :class "pt-1"}
