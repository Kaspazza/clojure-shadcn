    ...
    [:div {:class "space-y-4"}
     [helpers/api-component-card
      {:component-name "radio-group"
       :link {:href "https://www.radix-ui.com/primitives/docs/components/radio-group"
              :label "Radix Radio Group Docs"}
       :description
       "Container for mutually-exclusive options. Handles keyboard navigation and selected value management. Additional props are forwarded to Radix RadioGroup.Root."
       :props [{:name ":value"
                :type "string"
                :default nil
               ...
               {:name ":on-value-change"
                :type "function"
                :default nil
                :description "Callback when selected value changes: (fn [value] ...)."}
               {:name ":disabled"
                :type "boolean"
                :default nil
               ...
               {:name ":required"
                :type "boolean"
                :default nil
                :description "Marks group as required for forms."}
               {:name ":name"
                :type "string"
                :default nil
               ...
               {:name "additional props"
                :type "map entries"
                :default nil
                :description "Forwarded to Radix RadioGroup.Root."}]}]
     [helpers/api-component-card
      {:component-name "radio-group-item"
       ...
       :props [{:name ":value"
               ...
               {:name ":id"
                :type "string"
                :default nil
                :description "ID for associated label :html-for."}
               {:name ":disabled"
                :type "boolean"
                :default nil
...
(defstory
 ...
 []
 (r/as-element (helpers/wrap-component [:div {:class "p-6 space-y-2"}
...
(defstory RadioGroupPlayground
          ...
          {:args {:default-value "comfortable"
          ...
          (r/as-element (helpers/wrap-component
                         [:div {:class "p-6"}
                          [sut/radio-group
                           args
                           [:div {:class "flex items-center gap-2"}
                            [sut/radio-group-item {:id "density-default-playground"
                                                   :value "default"}]
                            [label/label {:html-for "density-default-playground"}
                             "Default"]]
                           [:div {:class "flex items-center gap-2"}
                            [sut/radio-group-item {:id "density-comfortable-playground"
                                                   :value "comfortable"}]
                            [label/label {:html-for "density-comfortable-playground"}
                             "Comfortable"]]]])))
