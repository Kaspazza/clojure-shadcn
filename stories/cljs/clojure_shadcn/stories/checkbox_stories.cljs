     ...
     [:div {:class "space-y-4"}
      [helpers/api-component-card
       {:component-name "checkbox"
        ...
        :description
        "Checkbox component built on top of Radix UI checkbox. Supports controlled and uncontrolled modes, indeterminate state, and aria-invalid styling for form validation."
        :props
        [{:name ":checked"
          :type "boolean | :indeterminate | \"indeterminate\""
          :default nil
          :description
          "Controlled checked state. Use true/false for checked/unchecked, or :indeterminate keyword (string also accepted) for a partial selection state."}
         {:name ":default-checked"
          :type "boolean"
          :default nil
          :description
          "Uncontrolled default checked state. Use when you don't need to control the state externally."}
         {:name ":on-checked-change"
          :type "function"
          :default nil
          :description
          "Callback when checked state changes: (fn [checked?] ...). Receives true, false, or :indeterminate."}
         {:name ":disabled"
          :type "boolean"
          :default nil
          :description
          "Disables the checkbox. Renders with reduced opacity and cursor-not-allowed."}
         {:name ":required"
          :type "boolean"
          :default nil
         ...
         {:name ":name"
          ...
          :default nil
          :description
          "Form field name. Used when checkbox is inside a <form> for native form submission."}
         {:name ":value"
          :type "string"
          :default nil
          :description "Form field value submitted with the name when checked."}
         {:name ":aria-invalid"
          :type "boolean"
          :default nil
          :description
          "Marks the checkbox as invalid. Applies destructive ring and border styling for form validation errors."}
         {:name ":class"
          :type "string"
          :default nil
          :description "Additional Tailwind classes merged with default styling."}]}]
      [:div {:class "border rounded-lg p-4 bg-amber-500/10 border-amber-500/30 mb-4"}
       ...
       [:ul {:class "text-xs text-muted-foreground space-y-1 list-disc pl-4"}
        ...
        [:li
         "For controlled usage, always pair :checked with :on-checked-change to avoid a read-only checkbox."]
        [:li
         "Use :indeterminate keyword for the indeterminate state. The component handles Radix JS interop in both directions — callbacks also return :indeterminate, never the raw string."]
        [:li
         "Use :aria-invalid true to show error styling — pair with Field and field-error for full validation UX."]]]]])))
  ...
  (r/as-element
   (helpers/wrap-component
    [:div {:class "p-6 space-y-6"}
     ...
     [:div {:class "space-y-3"}
      [:p {:class "text-sm font-medium"}
       "Invalid checkbox inside Field with error messages"]
      [field/field {}
       [field/field-content {}
        [:div {:class "flex items-center gap-2"}
     ...
     [:form {:on-submit (fn [e]
      ...
      [:div {:class "space-y-4"}
       ...
       [:button
        {:type "submit"
         :class
         "mt-4 rounded-md bg-primary px-4 py-2 text-sm text-primary-foreground hover:bg-primary/90"}
        "Submit form"]]]])))
      ...
      (let [all-tasks [{:id "TASK-001"
        ...
        (fn []
          (let [all-ids (set (map :id all-tasks))
                ...
                toggle-all! (fn [checked?] (reset! selected (if checked? all-ids #{})))
                toggle-row! (fn [id checked?] (swap! selected (if checked? conj disj) id))]
            (helpers/wrap-component
             [:div {:class "p-6"}
              ...
              [table/table {}
               [table/table-header {}
                [table/table-row {}
                 ...
                 [table/table-head {}
                  "Status"]
                 [table/table-head {:class "text-right"}
                  "Priority"]]]
               [table/table-body {}
                ;; doall: @selected is derefed while realizing the seq; a
                ;; lazy seq would realize outside the reactive context
                ;; and not re-render.
                (doall (for [{:keys [id title status priority]} all-tasks]
                         ^{:key id}
                         [table/table-row {:class (when (contains? @selected id) "bg-muted/50")}
                          [table/table-cell {}
                           [sut/checkbox {:checked (contains? @selected id)
                                          :on-checked-change (partial toggle-row! id)
                                          :aria-label (str "Select " id)}]]
                          [table/table-cell {:class "font-medium"}
                           id]
                          [table/table-cell {}
                           title]
                          [table/table-cell {}
