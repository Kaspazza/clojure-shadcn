...
(defstory ApiReference []
  (r/as-element (helpers/wrap-component
    [:div {:class "space-y-4 p-6 max-w-4xl"}
     [helpers/api-component-card {:component-name "navigation-menu" :link {:href "https://www.radix-ui.com/primitives/docs/components/navigation-menu" :label "Radix Navigation Menu Docs"} :description "Radix Root wrapper that manages focus, keyboard navigation, and content state. By default it appends one navigation-menu-viewport after supplied children." :props [{:name ":viewport" :type "boolean" :default "true" :description "When false, content is positioned directly and no automatic viewport is appended."} {:name ":class / Root props" :type "string / map entries" :default nil :description "Classes are merged and all remaining normalized Radix Root props forwarded."}]}]
     [helpers/api-component-card {:component-name "navigation-menu-list / navigation-menu-item" :description "Radix List and Item structural wrappers. List must be inside Root; triggers, links, and content belong within Items." :props [{:name ":class / primitive props" :type "string / map entries" :default nil :description "Merged and forwarded to the corresponding Radix primitive."}]}]
     [helpers/api-component-card {:component-name "navigation-menu-trigger / navigation-menu-content / navigation-menu-link" :description "Accessible trigger with automatic decorative chevron, associated content panel, and navigation link. Radix owns trigger/content ARIA and keyboard behavior." :props [{:name ":class / primitive props" :type "string / map entries" :default nil :description "Merged and forwarded. Link accepts Radix Link props such as :active and :as-child plus anchor props."}]}]
     [helpers/api-component-card {:component-name "navigation-menu-viewport / navigation-menu-indicator" :description "Animated content viewport and optional active-item indicator. Root creates the viewport automatically unless :viewport is false; avoid adding a second one." :props [{:name ":class / primitive props" :type "string / map entries" :default nil :description "Merged and forwarded to Viewport or Indicator."}]}]
     [helpers/api-component-card {:component-name "navigation-menu-trigger-style" :description "Public style helper for links that should visually match triggers." :props [{:name "classes" :type "variadic class values" :default nil :description "Merged after the default trigger classes; returns a class string."}]}]])))
