...
(defn ^:export Installation
  ...
  (r/as-element
   [helpers/installation-scene
    {:description "Controlled theme toggle button for switching between light and dark themes."
     ...
     [:div {:class "space-y-4"}
      [helpers/api-component-card
       {:component-name "theme-toggle"
        :description "Controlled theme switch button. Pure presentation: reads nothing from global state; the caller passes :theme and :on-toggle."
        :props [{:name ":theme"
                 :type "keyword"
                 :default ":light"
                ...
                {:name ":class"
                 :type "string"
                 :default nil
                 :description "Extra Tailwind classes merged with the base ones via tailwind-merge."}]}]
