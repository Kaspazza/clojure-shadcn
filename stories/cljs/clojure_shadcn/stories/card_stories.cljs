...
(defstory ApiReference []
  (r/as-element (helpers/wrap-component
    [:div {:class "space-y-4 p-6 max-w-4xl"}
     [helpers/api-component-card {:component-name "card" :description "Visual card root rendered as a div. Compose the region components below as needed; the API does not impose heading or landmark semantics." :props [{:name ":class" :type "string" :default nil :description "Classes merged with card defaults."} {:name "additional props" :type "map entries" :default nil :description "Normalized and forwarded to the div."}]}]
     [helpers/api-component-card {:component-name "card-header / card-title / card-description" :description "Header grid plus unopinionated div elements for title and supporting text. Use an actual heading element inside card-title when document hierarchy requires one." :props [{:name ":class / additional props" :type "string / map entries" :default nil :description "Merged and forwarded to each underlying div."}]}]
     [helpers/api-component-card {:component-name "card-action / card-content / card-footer" :description "Optional action, body, and footer layout regions. card-action is positioned by card-header's container grid and should therefore be nested in card-header." :props [{:name ":class / additional props" :type "string / map entries" :default nil :description "Merged and forwarded to each underlying div."}]}]])))
(defstory CardDemo []
  (r/as-element (helpers/wrap-component
