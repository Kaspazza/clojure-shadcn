...
(defn- principle-card
  ...
  [:article {:class "rounded-lg border bg-card p-4"}
   [:p {:class "mb-2 text-xs font-medium uppercase tracking-wider text-primary"}
    eyebrow]
   [:h3 {:class "mb-2 font-semibold text-card-foreground"} title]
   [:p {:class "text-sm leading-6 text-muted-foreground"} body]])

(defn- callout
  [title & children]
  [:div {:class "rounded-lg border border-primary/20 bg-primary/5 p-4"}
   [:h3 {:class "mb-2 text-sm font-semibold text-foreground"} title]
   ...
   (helpers/wrap-component
    [:main {:class "mx-auto max-w-4xl space-y-10 p-6"}
     [:header {:class "space-y-4"}
      [badge/badge {:variant :secondary} "Rationale"]
      [:h1 {:class "text-3xl font-bold tracking-tight md:text-4xl"}
       "Own the UI without rebuilding the hard parts"]
      [:p {:class "max-w-3xl text-base leading-7 text-muted-foreground"}
       "clojure-shadcn adapts the shadcn/ui copy-first idea for ClojureScript and Reagent. It reuses mature browser behavior, keeps visual decisions explicit, presents a Clojure-shaped API, and then gives the source to the application team."]]

     [section "The problem is coupling, not a lack of widgets"
      [:p {}
       "A button is easy to draw. Reliable dialogs, menus, comboboxes, and overlays require focus management, keyboard interaction, ARIA semantics, dismissal behavior, and consistent visual states."]
      [:p {}
       "Full UI suites solve many of those problems, but often ship behavior, styling, design policy, providers, and versioning as one decision. Building everything locally restores control but repeats specialized accessibility work. Headless primitives solve behavior while leaving every visual decision to each team."]
      [callout "The project chooses a middle path"
       [:p {}
     ...
     [section "A glue layer, not another framework"
      [:p {}
     ...
     [section "Why copy the source?"
      ...
      [callout "Forking is the workflow"
       [:p {}
     ...
     [section "Composition over configuration"
      [:p {}
     ...
     [section "Accessible foundations, not an accessibility guarantee"
      [:p {}
     ...
     [section "The project contract"
      [:div {:class "grid gap-6 md:grid-cols-2"}
       [:div {:class "space-y-3"}
        [:h3 {:class "font-semibold text-foreground"} "What we aim to provide"]
        [bullet-list
       ...
       [:div {:class "space-y-3"}
        [:h3 {:class "font-semibold text-foreground"} "What we do not promise"]
        [bullet-list
