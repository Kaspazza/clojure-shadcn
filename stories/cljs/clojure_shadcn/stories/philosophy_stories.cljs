   ...
   (helpers/wrap-component
    [:main {:class "mx-auto max-w-4xl space-y-10 p-6"}
     ...
     [section
      ...
      [:p {}
       "Most UI solutions for that are frameworks and as with every framework it is easy to start with, but far from simple. As your needs will grow for components outside of available components or you find yourself wanting to adjust one of the components it will give you a lot of pain as it has coupled multiple areas and made a lot of decisions for you. Before you know it, you have so many components, global hooks/scripts, css running for it that changing to other UI system is a giantic cost. On other hand there are libraries solving specific problems, like headless components, css framework utilties"]
      [callout
